...
import { getMoreProfesores, getProfesores } from "../hooks/useFirebaseFunctions";
...
const MenuProfesores = ({ navigation }: PropsNavigation) => {
  ...
  const moreData = async() => {
    if(!lastDocIsLoad){
      ...
      setProfesores([...profesores, ...getData.profesores]);
      setLastDoc(getData.lastVisible);
      getData.profesores.length==0?setLastDocIsLoad(true):setLastDocIsLoad(false);
    }
  }
  ...
  return (
    <Center flex="1">
      <Input placeholder="Filtar por Nombre" size="md" w="90%" mt="1" shadow="1" />
